import tensorflow as tf
import pandas as pd
import numpy as np
from src.train import *
from src.evaluate import *
from src.preprocessing import *
from src.model import *

def main():
    # Load the model
    model = build_imputation_model(X_train.shape[1:], F)
    model.compile(optimizer='adam', loss=MaskedMSE(F))
    model.summary()
    #train_model()   #for start training
    model.load_weights('./models/pems_random.keras')
  
    # ---------- Inference ----------
    preds = model.predict(X_test, batch_size=32)
    imputed_scaled = reconstruct_from_windows_weighted(preds=preds, T=len(test_scaled), window=window, F=F, stride=1, window_type="hanning")
    imputed_scaled[test_mask == 1] = test_scaled[test_mask == 1]
    imputed = scaler.inverse_transform(imputed_scaled)
    
    # Evaluate the model on the test set
    rmse, mae = compute_errors(imputed, test_raw, test_eval_mask)
    mape = compute_smape(imputed, test_raw, test_eval_mask)
    
    print(f"\nFINAL → RMSE: {rmse:.4f}   MAE: {mae:.4f}  MAPE: {mape:.4f}")
    
if __name__ == "__main__":
    main()
    
